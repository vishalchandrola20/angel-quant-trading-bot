...
def analyze_single_day(csv_path: Path) -> dict | None:
    ...
    trade_slice["total_pnl_bar"] = (entry_ce - trade_slice["ce_close"]) + (entry_pe - trade_slice["pe_close"])
    
    max_profit_trade_row = trade_slice.loc[trade_slice["total_pnl_bar"].idxmax()]
    max_profit_trade_time = max_profit_trade_row["ts"]
    max_profit_trade = max_profit_trade_row["total_pnl_bar"]
    max_loss_trade = trade_slice["total_pnl_bar"].min()

    # --- Full-Day P&L (relative to entry) ---
    df["total_pnl_bar_day"] = (entry_ce - df["ce_close"]) + (entry_pe - df["pe_close"])
    
    max_profit_day_row = df.loc[df["total_pnl_bar_day"].idxmax()]
    max_profit_day_time = max_profit_day_row["ts"]
    max_profit_day = max_profit_day_row["total_pnl_bar_day"]

    max_loss_day_row = df.loc[df["total_pnl_bar_day"].idxmin()]
    max_loss_day_time = max_loss_day_row["ts"]
    max_loss_day = max_loss_day_row["total_pnl_bar_day"]
